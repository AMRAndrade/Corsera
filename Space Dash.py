...
space_data = pd.read_csv("spacex_launch_dash.csv")
max_payload = space_data['Payload Mass (kg)'].max()
min_payload = space_data['Payload Mass (kg)'].min()
...
def get_pie_chart(entered_site):    
    if entered_site == 'ALL':
        fig = px.pie(space_data, 
                     values='class', 
                     names='Launch Site', 
                     title='Total Success Launches By Site')        
    else:
        filtered_df = space_data[space_data['Launch Site'] == entered_site]
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        filtered_df = space_data[(space_data['Payload Mass (kg)'] >= int(payload_range[0])) &
                                (space_data['Payload Mass (kg)'] <= int(payload_range[1]))
                               ]
        ...
        filtered_df = space_data[(space_data['Launch Site'] == entered_site) & 
                                (space_data['Payload Mass (kg)'] >= int(payload_range[0])) &
                                (space_data['Payload Mass (kg)'] <= int(payload_range[1]))
                               ]
